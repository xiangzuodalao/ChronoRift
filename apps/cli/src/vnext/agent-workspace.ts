...
export interface AgentWorkspaceBinding {
  ...
  readonly agentGitDirectory: string;
...
interface AgentWorkspaceState {
  ...
  readonly parentWorkspaceDirectory: string;
...
const writeSnapshot = async (
  ...
  if (!existing) await createPrivateDirectory(directory);
...
export class AgentWorkspaceManager {
  ...
  readonly #git: HostGitPort;
  readonly #operation = new AgentWorkspaceGate();
  #poisoned = false;
  ...
  private serialize<T>(
    ...
    return this.#operation.run(operation, signal);
  }
  ...
  public create(
    ...
    return this.serialize(() => this.createWorkspace(agentId, parentAgentId));
  }
  ...
  private async createWorkspace(
    ...
    const parentWorkspaceDirectory =
      parentAgentId === "/root"
        ? this.options.rootWorkspaceDirectory
        : this.state(parentAgentId).binding.workspaceDirectory;
    const files = await collect(parentWorkspaceDirectory);
    ...
    const scratch = join(resourceDirectory, "sandbox-artifacts");
    await createPrivateDirectory(scratch);
    const agentGitDirectory = join(scratch, "repository");
    await createPrivateDirectory(agentGitDirectory);
    await this.#git.initializeRepository({
      directory: hostBaselineGitDirectory,
      bare: true,
    });
    await this.#git.initializeRepository({
      directory: agentGitDirectory,
      bare: false,
    });
    ...
    await this.commitSnapshot(
      mergeBase.directory,
      files,
      join(agentGitDirectory, ".git"),
      join(agentGitDirectory, ".git", "index"),
      true,
    );
    // Each agent has its own sanitized backing repository. Its mutable Git
    // metadata is in that agent's sandbox scratch, never in the Host baseline.
    await this.#git.addDetachedNoCheckoutWorktree({
      repositoryRoot: agentGitDirectory,
      worktreePath: workspaceDirectory,
      commit: baselineCommit,
    });
    await chmod(workspaceDirectory, 0o700);
    await writeSnapshot(workspaceDirectory, files, true);
    const worktreeGit = join(
      agentGitDirectory,
      ".git",
      "worktrees",
      "workspace",
  ...
  public applyTurn(
    ...
    return this.serialize(
      () => this.applyRecordedTurn(agentId, turnId, signal),
      signal,
    );
  }
  ...
  private async applyRecordedTurn(
    ...
    if (turnId === state.appliedTurnId)
      return {
        ...
        rootSourceHash: selectedTreeSha256(
          await collect(state.parentWorkspaceDirectory),
        ),
      };
    ...
    const rootFiles = await collect(state.parentWorkspaceDirectory);
    ...
    const root = await pinRoot(state.parentWorkspaceDirectory);
    try {
      if (
        selectedTreeSha256(await collect(state.parentWorkspaceDirectory)) !==
        beforeHash
      )
        throw new Error("Root source changed before patch application");
      // Preparation is cancellable. Once candidate writes start, finish the
      // transaction or roll it back before releasing the owning workspace.
      signal?.throwIfAborted();
      try {
        await this.install(root, paths, merged, "apply");
        await assertRootBound(state.parentWorkspaceDirectory, root);
        if (
          selectedTreeSha256(await collect(state.parentWorkspaceDirectory)) !==
          afterHash
        )
