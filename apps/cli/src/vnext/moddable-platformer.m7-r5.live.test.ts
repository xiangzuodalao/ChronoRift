/**
 * R5 is a fresh operator attempt over the frozen R4 protocol-v1 composition.
...
const installM7R5ProtocolCompatibilityV1 = (): void => {
  const mode = process.env.CHRONORIFT_M7_R5_LIVE_MODE;
  if (
    mode !== "pre-agent-dry-run" &&
    mode !== "no-agent-preflight" &&
    mode !== "r5-live"
  ) {
    throw new Error("M7 R5 composer received an invalid operator mode");
  }
  ...
  if (R4_PROTOCOL_ENVIRONMENT_SUFFIXES.length !== 39) {
    throw new Error("M7 R5 protocol environment whitelist changed");
  }
  for (const suffix of R4_PROTOCOL_ENVIRONMENT_SUFFIXES) {
    const value = process.env[`CHRONORIFT_TEST_M7_R5_${suffix}`];
    if (value === undefined || value.length === 0) {
      throw new Error(`M7 R5 composer is missing protocol input ${suffix}`);
    }
    process.env[`CHRONORIFT_TEST_M7_R4_${suffix}`] = value;
  }
  process.env.CHRONORIFT_M7_R4_LIVE_MODE =
    mode === "r5-live" ? "r4-live" : mode;
};

installM7R5ProtocolCompatibilityV1();
