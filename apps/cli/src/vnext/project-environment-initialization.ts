...
export const composeProjectEnvironmentInitializationPromptV2 = (input: {
  ...
  composeProjectEnvironmentInitializationPromptV1(input)
    ...
    .concat(
      "\nAuthor a manifest/SDK/observation V2 adapter. Inspect project source first, replace every dynamic-placeholder identifier, and implement only source-derived node and Signal semantics. Preserve the scaffold's small payload schemas when they honestly represent the project; do not invent extra fields. Keep manifest engine.versionRequirement as the protocol literal 4.7.x even though the realized managed binary is exact 4.7.1. After the files are coherent, call project_adapter_finalize_v2 exactly once to update schema hashes and fixed protocol fields, then finish. Ready requires one lossless declared dynamic trace with entity-bound state and event observations across exactly consecutive incarnations. Do not run Godot or perform conformance yourself because the Host performs authoritative validation after this turn.",
    );
