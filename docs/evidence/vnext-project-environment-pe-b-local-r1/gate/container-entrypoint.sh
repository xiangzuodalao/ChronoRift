...
mkdir /sys/fs/cgroup/driver/runner /sys/fs/cgroup/driver/chronorift-vnext-godot
...
exec runuser -u ubuntu -- env \
  ...
  CHRONORIFT_TEST_PE_B_LIVE=1 \
  CHRONORIFT_TEST_PE_B_PROVIDER=openai-codex \
  CHRONORIFT_TEST_PE_B_MODEL=gpt-5.6-luna \
  CHRONORIFT_TEST_PE_B_THINKING_LEVEL=max \
  CHRONORIFT_TEST_PE_B_HOST_CONFIG=/workspace/.chronorift/pe-b-live/host-config.v1.json \
  CHRONORIFT_TEST_PE_B_AGENT_DIR=/pi-agent \
  CHRONORIFT_TEST_PE_B_EVIDENCE_OUTPUT_DIR=/evidence \
  /usr/lib/chronorift-ci/bin/node-22.23.1 \
  /workspace/node_modules/vitest/vitest.mjs run \
  --config /workspace/vitest.live.config.ts \
  /workspace/apps/cli/src/vnext/project-environment-pe-b.live.test.ts
