...
Replace every `dynamic-placeholder` identifier and matching stub in the reference. Declare between 1 and 32 real headless launch targets with exactly one default and strict empty-parameter schemas. The default target must describe the project's realized main scene. When the Host names a selected target, it must also be declared; publication validates the default plus that selected target when they differ, while every other target remains explicitly declared but unvalidated. Keep `engine.versionRequirement` as the protocol literal `4.7.x`; the realized exact 4.7.1 binary is a separate Host receipt. Implement lifecycle, clock, runtime errors, entity/state/event projection, and capture; declare input, snapshot, restore, render capture, and alignment unsupported unless they are actually exercised. Keep the existing schema files and change their schema IDs as needed. Preserve their small payload shapes when they honestly describe the project. Once the semantic declarations and GDScript agree, call `project_adapter_finalize_v2` once. It rebuilds manifest schema declarations and SHA-256 values from the actual schema documents, binds the default scene to the realized project main scene, forces every target to headless rendering, verifies any Host-selected target is declared, restores fixed SDK/engine protocol fields, and freezes the candidate for the rest of the turn. It preserves non-default target scenes. If it rejects a semantic reference, fix that exact reference and retry; do not manually repair hashes or schema paths. After it succeeds, finish immediately; mutating tools are deliberately unavailable. It does not validate project semantics.
