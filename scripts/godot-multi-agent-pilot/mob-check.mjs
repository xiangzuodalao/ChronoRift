...
export const SOURCE_COMMIT = "711822a319c4333a8740522f3c71e97783199fb0";
export const SOURCE_TREE = "e80cfdab1b1c2b917bd1fe65d5aeaec5cf292539";
export const PROJECT_PREFIX = "3d/squash_the_creeps";
const CHECKER_PATH = "__mob_orientation_independent_check.gd";
...
export async function snapshotBaseline(project) {
  ...
  requireValue(
    ...
    "--project must be the pinned squash_the_creeps subtree",
  );
  requireValue(
...
// Preserve the existing case's narrow oracle: three seeded Mob initializations.
// It does not establish end-to-end spawn, player collision, or gameplay behavior.
export function assess(importProcess, runtime) {
  try {
    ...
    const lines = runtime.process.stdout
      .split(/\r?\n/u)
      .filter((line) => line.startsWith("CHRONORIFT_MOB_EVAL="));
    requireValue(lines.length === 1, "Missing or duplicate evaluator output");
    const record = JSON.parse(lines[0].slice("CHRONORIFT_MOB_EVAL=".length));
    requireValue(
      record.schemaVersion === 1 &&
        Array.isArray(record.observations) &&
        record.observations.length === 3,
      "Malformed Mob observations",
    );
    const observations = record.observations;
    for (const [index, observation] of observations.entries()) {
      requireValue(
        observation.seed === 7301 + index * 101 &&
          [
            observation.upAlignment,
            observation.velocityY,
            observation.horizontalSpeed,
            observation.minSpeed,
            observation.maxSpeed,
          ].every(Number.isFinite),
        "Malformed Mob observation or unexpected seed",
      );
    }
    const accepted = observations.every(
      (o) =>
        o.upAlignment >= 0.999999 &&
        Math.abs(o.velocityY) <= 0.000001 &&
        o.horizontalSpeed >= o.minSpeed - 0.000001 &&
        o.horizontalSpeed <= o.maxSpeed + 0.000001,
    );
    requireValue(
    ...
    return {
      ...
      observations,
      scope:
        "Three seeded Mob initializations; orientation, vertical velocity and declared horizontal speed range. No full gameplay or random distribution proof.",
    };
  } catch (error) {
...
export async function check(options) {
  ...
  try {
    ...
    const checker = await readOrdinaryFile(
      fileURLToPath(new URL("mob-independent-check.gd", import.meta.url)),
      1024 * 1024,
    );
    ...
    privateRoot = await mkdtemp(join(tmpdir(), "chronorift-mob-check-"));
    ...
    if (patch.byteLength) {
      ...
      const patchFile = join(workspace, "__mob_candidate.patch");
      requireValue(
        !sourceFiles.some(
          (file) => file.relativePath === "__mob_candidate.patch",
        ),
  ...
  await save(join(options.output, "result.json"), {
    schemaVersion: 1,
    kind: "mob-orientation-independent-check",
    startedAt,
